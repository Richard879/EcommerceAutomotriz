...
class DescuentosOtorgadosController extends Controller
{
    ...
    public function GetListPedidoDistribuido(Request $request)
    {
        $nidempresa         =   $request->nidempresa;
        $nidsucursal        =   $request->nidsucursal;
        $nidvendedor        =   $request->nidvendedor;
        ...
        $data = DB::select('exec [usp_DescuentosOtorgados_GetPedidoDistribuidos] ?, ?, ?, ?, ?, ?',
                                            [
                                                $nidempresa,
                                                $nidsucursal,
                                                $nidvendedor,
        ...
        if ($opcion == 1) {
            ...
            return ['arrayPedidosDistribuido'=>$data];
        }
    }

    public function SetUpdDocNumNC(Request $request)
    {
        if (!$request->ajax()) return redirect('/');
        ...
        try{
            ...
            foreach($arrayvehiculos as $ep=>$value)
            {
                $data =  DB::select('exec [usp_Cotizacion_SetUpdDocNumNC] ?, ?, ?',
                                                                    [
                                                                        $value['nIdCabeceraCotizacion'],
                                                                        $value['nDocNumNC'],
                                                                        Auth::user()->id
